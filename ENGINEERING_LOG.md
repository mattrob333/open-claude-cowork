# Engineering Log - Get Shit Done Bot

## Session: January 25, 2026

### Overview
Major rebranding to "Get Shit Done Bot", fixed document Knowledge Base context injection, added Quick Actions system, and prepared for production deployment.

---

## Changes Made

### 1. Rebranding to "Get Shit Done Bot"

**Files Modified:**
- `renderer/index.html` - Updated page title
- `renderer/src/components/Sidebar.tsx` - New logo (checkbox icon) + "GET SHIT **DONE.**" branding
- `renderer/src/pages/LandingPage.tsx` - Updated header and footer
- `renderer/src/pages/LoginPage.tsx` - Updated header
- `README.md` - Updated project name and description

### 2. Document Knowledge Base - Context Injection Fix

**Problem:** Uploaded documents were not being used by the AI. Chunks weren't stored correctly, and even when stored, they weren't being injected into the AI's context.

**Root Cause:** 
- `storeChunks` function was attempting to insert non-existent columns (`user_id`, `page_number`)
- `fetchChunksForDocuments` was selecting non-existent `page_number` column

**Files Modified:**
- `server/lib/supabase.js` - Fixed chunk storage and retrieval
- `server/server.js` - Added logging for context injection debugging

### 3. Quick Actions System (Renamed from Workflows)

**Changes:**
- Renamed "Save as Workflow" button to "Save as Quick Action"
- Renamed "Send to Studio" button to "Save as Quick Action"  
- Hidden raw `WORKFLOW_EXTRACTION_REQUEST` prompt from users
- Backend now intercepts `__QUICK_ACTION_EXTRACT__` trigger and expands it server-side
- Updated WorkflowPanel empty state text

**Files Modified:**
- `renderer/src/components/ChatArea.tsx`
- `renderer/src/components/Workflow/WorkflowPanel.tsx`
- `server/server.js`

### 4. AI-Powered Session Titles

**Feature:** New sessions now get a 2-4 word title generated by AI after the first response.

**Files Modified:**
- `renderer/src/services/chatService.ts` - Added `generateSessionTitle()` function
- `renderer/src/App.tsx` - Calls title generation after first message exchange
- `server/server.js` - Added `/api/generate-title` endpoint

### 5. Delete Button Fixes

**Problem:** Delete buttons in Knowledge Base weren't working.

**Root Cause:** CORS wasn't allowing DELETE method, and storage paths were incorrect.

**Files Modified:**
- `server/server.js` - Added DELETE to CORS allowed methods
- `server/routes/documents.js` - Fixed storage path for deletion
- `server/lib/supabase.js` - Updated deleteDocument to use full storage path
